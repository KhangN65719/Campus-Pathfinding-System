...
CXXFLAGS = \
	...
	-std=c++2a -I. -Isrc/ -Ilib/ -O2 -g -fno-omit-frame-pointer \
...
build/%.o: tests/%.cpp src/graph.h
	mkdir -p build && $(CXX) $(CXXFLAGS) -c $< -o $@
build/%.o: src/%.cpp src/graph.h
	mkdir -p build && $(CXX) $(CXXFLAGS) -c $< -o $@
...
SOURCES := $(filter-out main server,$(basename $(notdir $(SOURCE_FILES))))
